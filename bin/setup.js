...
if(process.argv.length < 3) {
  console.log("Please specify the path to the module that you want to " +
    "configure. (Usually: '.')");
  process.exit(0);
}

var module_path = path.resolve(process.argv[2]);

if(!fs.statSync(module_path).isDirectory()) {
  console.log("'" + module_path + "' is not a directory! Specify the path to " +
    "the module");
  process.exit(0);
}

console.log("Configuring '" + module_path + "'");

var config_file_path = path.resolve(module_path, "config", "config.json");
var config_file_backup_path = path.resolve(module_path, "config",
  "config.json.bak");

try {
  config_values = require(path.resolve(module_path, "config", "config"));
}
catch(e) {
  config_values = {};
}

var config_fields = require(path.resolve(module_path, "config",
  "config_fields"));
...
for(var config_field_index = 0; config_field_index < num_config_fields;
  ...
  config_field_dict[config_fields[config_field_index].key] =
    config_fields[config_field_index];
